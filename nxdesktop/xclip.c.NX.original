			...
			uint32 length)
{
	...
	DEBUG_CLIPBOARD((stderr, "xclip_provide_selection: requestor=0x%08x, target=%s, property=%s, length=%u\n", 
...
xclip_refuse_selection(XSelectionRequestEvent * req)
{
	...
	DEBUG_CLIPBOARD((stderr, "xclip_refuse_selection: requestor=0x%08x, target=%s, property=%s\n",
			 (unsigned) req->requestor, XGetAtomName(g_display, req->target),
...
{
	DEBUG_CLIPBOARD((stderr, "xclip_send_data_with_convert: target=%s, size=%u\n",
            ...
            target == format_unicode_atom || target == format_utf8_string_atom)
        {
                ...
                if (target == format_string_atom)
                {
                        ...
                        if (cd == (iconv_t) - 1)
                        {
                                DEBUG_CLIPBOARD((stderr, "Locale charset %s not found in iconv. Unable to convert clipboard text.\n", locale_charset));
                                return False;
                        }
                ...
                {
                        DEBUG_CLIPBOARD((stderr, "Sending Unicode string of %d bytes\n",
        ...
        if (target == format_string_atom)
        {
                ...
                DEBUG_CLIPBOARD((stderr, "Translating linebreaks before sending data\n"));
...
xclip_handle_SelectionNotify(XSelectionEvent * event)
{
	...
	DEBUG_CLIPBOARD((stderr, "xclip_handle_SelectionNotify: selection=%s, target=%s, property=%s\n", 
	...
	if (res != Success)
	{
		DEBUG_CLIPBOARD((stderr, "XGetWindowProperty failed!\n"));
		goto fail;
	}

	if (type == incr_atom)
	{
                DEBUG_CLIPBOARD((stderr, "Received INCR.\n"));
        ...
        if (event->target == targets_atom)
        {
                ...
                if (type != None)
                {
                        ...
                        for (i = 0; i < nitems; i++)
                        {
                                DEBUG_CLIPBOARD((stderr, "Target %d: %s\n", i, XGetAtomName(g_display, supported_targets[i])));
                                if (supported_targets[i] == format_string_atom)
                                {
                                        if (text_target_satisfaction < 1)
                                        {
                                                DEBUG_CLIPBOARD((stderr, "Other party supports STRING, choosing that as best_target\n"));
                                ...
                                else if (supported_targets[i] == format_unicode_atom)
                                {
                                        ...
                                        {
                                                DEBUG_CLIPBOARD((stderr, "Other party supports text/unicode, choosing that as best_target\n"));
                                                best_text_target = supported_targets[i];
                                ...
                                else if (supported_targets[i] == format_utf8_string_atom)
                                {
                                        ...
                                        {
                                                DEBUG_CLIPBOARD((stderr, "Other party supports UTF8_STRING, choosing that as best_target\n"));
                                                best_text_target = supported_targets[i];
			...
			|| (best_text_target == rdesktop_clipboard_formats_atom)))
                ...
                {
                        DEBUG_CLIPBOARD((stderr, "Unable to find a textual target to satisfy RDP clipboard text request\n"));
                        goto fail;
                }
        }
        else
...
xclip_handle_SelectionRequest(XSelectionRequestEvent * event)
{
	...
	DEBUG_CLIPBOARD((stderr, "xclip_handle_SelectionRequest: selection=%s, target=%s, property=%s\n",
			 XGetAtomName(g_display, event->selection),
			 XGetAtomName(g_display, event->target),
			 XGetAtomName(g_display, event->property)));
	...
	else 
	{
		/* All the following targets require an async operation with the RDP server
                ...
                {
                        DEBUG_CLIPBOARD((stderr, "Error: Another clipboard request was already sent to the RDP server and not yet responded. Refusing this request.\n"));
			...
			XFree(prop_return);
...
#else
                        DEBUG_CLIPBOARD((stderr, "Requested target unavailable due to lack of Unicode support. (It was not inTARGETS, so why did you ask for it?!)\n"));
                        xclip_refuse_selection(event);
                        return;
#endif
                }
                ...
                {
                        DEBUG_CLIPBOARD((stderr, "Requested target unavailable. (It was not in TARGETS, so why did you ask for it?!)\n"));
                        xclip_refuse_selection(event);
                        return; 
                }
...
{
	DEBUG_CLIPBOARD((stderr, "xclip_handle_SelectionClear\n"));
	xclip_notify_change();
	xclip_probe_selections();
}
...
xclip_handle_PropertyNotify(XPropertyEvent * event)
{
	...
	{
                DEBUG_CLIPBOARD((stderr, "x_clip_handle_PropertyNotify: g_waiting_for_INCR != 0\n"));
        ...
        {
		DEBUG_CLIPBOARD((stderr, "xclip_handle_PropertyNotify: getting fellow rdesktop formats\n"));
		xclip_probe_selections();
        }
}
#endif
...
ui_clip_handle_data(uint8 * data, uint32 length)
{
        ...
        if (length == 0)
	{
        ...
        {
		DEBUG_CLIPBOARD((stderr, "ui_clip_handle_data: BUG! I don't know how to convert selection target %s!\n", 
...
ui_clip_request_data(uint32 format)
{
	...
	DEBUG_CLIPBOARD((stderr, "Request from server for format %d\n", format));
